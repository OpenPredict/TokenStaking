...
// Farm distributes the OpenPredict rewards based on staked tokens to each user.
//
// Cloned from https://github.com/ltonetwork/uniswap-farming/blob/master/contracts/Farm.sol
// Modified by OpenPredict.
contract Farm is Ownable {
    ...
    uint256 public maxStakeAmount = 100000 ether; // max amount of staking token allowed.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.supply.add(_amount) <= maxStakeAmount, "deposit: max stake amount exceeded");
    ...
    function increaseMaxStakeAmount(uint amount) external onlyOwner {
